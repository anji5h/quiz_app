...
DATA_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../", "data")) # json files for the quiz questions
...
def load_questions(topic):
    ...
    random.shuffle(questions)  # Shuffles the order of questions

    for q in questions:
        correct_answer = q["answer"]
        options = q["options"]
        random.shuffle(options)  # Shuffles the options

        # Updates correct answer position after shuffling
        q["answer"] = correct_answer  # Still same text, just position changes
        q["options"] = options
